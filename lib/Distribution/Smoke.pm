...
sub _build_dist {
    ...
    my $cmd =
      "cpanm -l $ipath --verbose $dist->{name} --no-interactive > $lpath 2>&1";
    ...
    for my $base_dist ( @{ $self->dists } ) {
        ...
        for my $dist (@to_test) {
            ...
            if ( $pass_report->exists ) {
                ...
                my $cmd =
"cpanm -l $ipath --test-only --verbose $dist --no-interactive 2>&1";
                $self->log_verbose( "\tRunning", $cmd );
...
sub _resolve_reverse_dependencies {
    ...
    for my $base_dist ( @{ $self->dists } ) {
        if ( $base_dist->{file_based} ) {
            $self->log(
"\tSkipping $base_dist->{name}, currently cannot determine file-based dist names!"
            );
